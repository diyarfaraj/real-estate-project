  ...
  useEffect(() => {
    const fetchBookmarkedProperties = async () => {
      try {
        ...
        const data = await request(
          `/property/find/bookmarked-properties`,
          "GET",
          options
        );
      ...
      <div className={classes.wrapper}>
        <div className={classes.profile}>
          <div className={classes.updateDeleteControls}>
            <Link className={classes.updateBtn} to={`/update-profile`}>
              Update Profile
            </Link>
            {deleteModal && (
        ...
        <div className={classes.buttons}>
          <button
            className={`${classes.button} ${activeBtn === 0 && classes.active}`}
            onClick={() => setActiveBtn((prev) => 0)}
          >
            Listed properties
          </button>
          <button
            className={`${classes.button} ${activeBtn === 1 && classes.active}`}
            onClick={() => setActiveBtn((prev) => 1)}
          >
            Listed yachts
          </button>
          <button
            className={`${classes.button} ${activeBtn === 2 && classes.active}`}
            onClick={() => setActiveBtn((prev) => 2)}
          >
            Bookmarked properties
          </button>
          <button
            className={`${classes.button} ${activeBtn === 3 && classes.active}`}
            onClick={() => setActiveBtn((prev) => 3)}
          >
            Bookmarked yachts
          </button>
        </div>
          ...
          {activeBtn === 0 && (
            <>
              {listedProperties?.length > 0 && (
                <h2 className={classes.title}>Listed Properties</h2>
              )}
                ...
                {listedProperties?.length > 0 ? (
                  listedProperties?.map((listedProperty) => (
                    <div key={listedProperty._id} className={classes.property}>
                      <Link
                        to={`/propertyDetail/${listedProperty._id}`}
                      ...
                      <div className={classes.details}>
                        ...
                        <div className={classes.moreDetails}>
                          ...
                          <span>
                            {listedProperty?.sqmeters} sq. meters
          ...
          {activeBtn === 1 && (
            <>
              {listedYachts?.length > 0 && (
                <h2 className={classes.title}>Listed Yachts</h2>
              )}
              {listedYachts?.length > 0 ? (
          ...
          {activeBtn === 2 && (
            <>
              {bookmarkedProperties?.length > 0 && (
                <h2 className={classes.title}>Bookmarked Properties</h2>
              )}
              <div className={classes.properties}>
                {bookmarkedProperties?.length > 0 ? (
                  bookmarkedProperties?.map((bookmarkedProperty) => (
                    <div
                      key={bookmarkedProperty._id}
                      className={classes.property}
                    >
                      <Link
                        to={`/propertyDetail/${bookmarkedProperty._id}`}
                        className={classes.imgContainer}
                      >
                        <img
                      ...
                      <div className={classes.details}>
                        ...
                        <div className={classes.moreDetails}>
                          <span>
                            {bookmarkedProperty?.beds}{" "}
                            <FaBed className={classes.icon} />
                          </span>
                          <span>
                            {bookmarkedProperty?.sqmeters} sq. meters
                            <FaSquareFull className={classes.icon} />
                          </span>
                        </div>
                        <div className={classes.desc}>
          ...
          {activeBtn === 3 && (
            <>
              {bookmarkedYachts?.length > 0 && (
                <h2 className={classes.title}>Bookmarked Yachts</h2>
              )}
              {bookmarkedYachts?.length > 0 ? (
                <div className={classes.yachts}>
                ...
                <h2 className={classes.noListed}>
                  You have no bookmarked yachts
                </h2>
              )}
            </>
          )}
        </div>
        {error && <div className={classes.error}>There was an error!</div>}
      </div>
    </div>
  );
};
